...
class UpdateWorkshopUseCase {
  ...
  const UpdateWorkshopUseCase(this._workshopRepository, this._authRepository);

  /// Updates an existing workshop
  /// 
  /// Validates admin permissions and workshop data before update
  /// Returns [Result<Workshop>] with updated workshop on success or exception on failure
  Future<Result<Workshop>> execute(Workshop workshop) async {
    try {
      ...
      final existingWorkshopResult = await _workshopRepository.getWorkshopById(workshop.id);
      ...
      // Update workshop with current timestamp
      final workshopToUpdate = workshop.copyWith(
        updatedAt: DateTime.now(),
      );

      // Update workshop in repository
      final result = await _workshopRepository.updateWorkshop(workshopToUpdate);
      
      return result.fold(
        onSuccess: (updatedWorkshop) => Success(updatedWorkshop),
        onFailure: (exception) => Failure(exception),
      );
    } catch (e) {
      return Failure(UnknownException('워크샵 수정 중 오류가 발생했습니다: ${e.toString()}'));
    }
  }
